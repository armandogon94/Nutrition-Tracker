...
struct MockMealPlanningServiceTests {
    ...
    func addThenMove() async throws {
        ...
        let plan = try await sut.createPlan(weekStartDate: .now, userId: UUID(), name: "Test")
        ...
        let afterAdd = try await sut.currentPlan()
        ...
        let afterMove = try await sut.currentPlan()
        ...
        let afterRemove = try await sut.currentPlan()
    ...
    func checkRoundTrips() async throws {
        let sut = MockMealPlanningService()
        let list = try await sut.shoppingList()
